...
class UIHandler {
  constructor() {
    this.$window = $(window);
    this.topbuttonVisible = this.$window.scrollTop() > 100;
  ...
  initUI() {
    ...
    this.$window.on('scroll', () => this.scheduleScrollHandler());
    this.$window.on('resize', () => {
      this.navHeight = $('#head-panel').height();
      this.scheduleScrollHandler();
    });
  ...
  scheduleScrollHandler() {
    ...
    window.requestAnimationFrame(() => {
      this.scrollTicking = false;
      this.handleScroll(this.$goto_top, this.$stickyNav, this.navHeight);
    });
  }
...
$(() => {
  uiHandler.initUI()
    ...
    const handleStatusBeforeUnload = () => stopStatusPolling();

    const fetchStatus = () => {
      if (document.hidden || statusInFlight) {
        return;
      }
      ...
      $.ajax({
        ...
        error: (xhr) => {
          if (xhr.status === 400) {
            ...
            $(window).off('beforeunload', handleStatusBeforeUnload);
    ...
    const startStatusPolling = () => {
      ...
      statusInterval = setInterval(() => fetchStatus(), statusPollDelay);
    };

    fetchStatus();
    startStatusPolling();
    document.addEventListener('visibilitychange', handleStatusVisibilityChange);
    $(window).on('beforeunload', handleStatusBeforeUnload);
  }
});
