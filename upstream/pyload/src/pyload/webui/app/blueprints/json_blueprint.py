...
def add_package():
    ...
    try:
        ...
        if file.filename:
            ...
            safe_filename = secure_filename(os.path.basename(file.filename)) or "upload_file"
            upload_path = os.path.join(api.get_cachedir(), "upload")
            os.makedirs(upload_path, exist_ok=True)
            file_path = safejoin(upload_path, safe_filename)
            file.save(file_path)
...
def edit_package():
    ...
    try:
        ...
        storage_dir = os.path.realpath(api.get_config_value("general", "storage_folder"))
        requested_dir = os.path.realpath(os.path.join(storage_dir, pack_folder or ""))

        try:
            if os.path.commonpath([storage_dir, requested_dir]) != storage_dir:
...
def change_password():
    ...
    s = flask.session
    if s.get("role") != Role.ADMIN and user != s.get("name"):
        return jsonify(False), 403
