...
def get_file(path):
    ...
    requested_path = os.path.realpath(os.path.join(directory, unquote(path)))

    try:
        if os.path.commonpath([directory, requested_path]) != directory:
            flask.abort(403)
    except ValueError:
        flask.abort(403)
    ...
    return flask.send_file(requested_path, as_attachment=True)
...
def settings():
    ...
    if flask.session.get("role") == Role.ADMIN:
