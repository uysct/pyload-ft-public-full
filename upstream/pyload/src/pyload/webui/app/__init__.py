...
class App:
    ...
    def _configure_config(cls, app, develop):
        ...
        secret_key_path = os.path.join(data_dir, "webui.secret_key")
        if os.path.exists(secret_key_path):
            with open(secret_key_path, "r", encoding="utf-8") as fp:
                secret_key = fp.read().strip()
        else:
            secret_key = os.urandom(32).hex()
            with open(secret_key_path, "w", encoding="utf-8") as fp:
                fp.write(secret_key)
    ...
    def _configure_handlers(cls, app):
        ...
        # Dynamically set SESSION_COOKIE_SECURE and optionally enforce allowed hosts
        @app.before_request
        def set_session_cookie_secure():
            x_forwarded_proto = flask.request.headers.get("X-Forwarded-Proto", "")
            is_secure = (
                flask.request.is_secure
                or x_forwarded_proto.split(",")[0].strip().lower() == "https"
            )
            flask.current_app.config["SESSION_COOKIE_SECURE"] = is_secure

            allowed_hosts_raw = os.environ.get("PYLOAD_ALLOWED_HOSTS", "")
            allowed_hosts = {
            ...
            if allowed_hosts:
                request_host = flask.request.host.split(":", 1)[0].rstrip(".").lower()
    ...
    def _configure_session(cls, app):
        userdir = app.config["PYLOAD_API"].get_userdir()
        session_path = os.path.join(userdir, "data", "flask")
