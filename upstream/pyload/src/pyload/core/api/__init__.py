...
class Api:
    ...
    **pyLoads API**
    ...
    def parse_urls(self, html: Optional[str] = None, url: Optional[str] = None) -> dict[str, list[str]]:
        ...
        if url:
            if not _is_public_http_url(url):
                raise ServiceException("Refusing non-public or unsupported URL")
    ...
    def check_online_status_container(self, urls: list[str], container: str, data: bytes) -> OnlineCheck:
        ...
        upload_file = safejoin(upload_path, container)
        with open(upload_file, "wb") as th:
            th.write(data)

        return self.check_online_status(urls + [upload_file])
    ...
    def upload_container(self, filename: str, data: bytes) -> None:
        ...
        upload_file = safejoin(upload_path, filename)
        with open(upload_file, "wb") as th:
            th.write(data)

        self.add_package(upload_file, [upload_file], Destination.COLLECTOR)
    ...
    def getUserData(self, username: str, password: str) -> OldUserData:
        ...
        try:
            try:
                current_user = flask.g.user_info
            except AttributeError:
                current_user = flask.session
        except RuntimeError:
            current_user = None

        if (
            current_user
            and current_user.get("role") != Role.ADMIN
            and current_user.get("name") != username
        ):
            return OldUserData()
    ...
    # Intentionally not exported with a non-admin permission:
    # generic addon RPC execution stays admin-only via the default authorization model.
    @post
