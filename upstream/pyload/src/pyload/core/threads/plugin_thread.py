...
class PluginThread(Thread):
    ...
    def write_debug_report(self, pyfile):
        ...
        plugin_name = os.path.basename(pyfile.pluginname) or "unknown_plugin"
        dump_name = f"debug_{plugin_name}_{date}.zip"
        dump_filename = safejoin(self.pyload.tempdir, dump_name)
        ...
        try:
            ...
            with zipfile.ZipFile(dump_filename, "w") as zip:
                try:
                    plugin_dump_dir = safejoin(self.pyload.tempdir, plugin_name)
                    for entry in os.listdir(plugin_dump_dir):
                        try:
                            # avoid encoding errors
                            zip.write(
                                safejoin(plugin_dump_dir, entry),
                                os.path.join(plugin_name, entry),
                            )
                        except Exception:
                            pass
                ...
                info = zipfile.ZipInfo(
                    os.path.join(plugin_name, "debug_Report.txt"), time.gmtime()
                )
