        ...
        with zipfile.ZipFile(self.filename, "r") as z:
            ...
            for member in z.infolist():
                ...
                if _is_zip_symlink(member):
                    raise ArchiveError("Refusing to process zip symlink")

                self.files.append(safejoin(self.dest, member.filename))
    ...
    def extract(self, password=None):
        ...
        try:
            with zipfile.ZipFile(self.filename, "r") as z:
                ...
                selected_members = []
                for member in z.infolist():
                    if any(fnmatch.fnmatch(member.filename, exclusion)
                           for exclusion in self.excludefiles):
                        continue

                    if _is_zip_symlink(member):
                        raise ArchiveError("Refusing to extract zip symlink")

                    safejoin(self.dest, member.filename)
                    selected_members.append(member)

                z.extractall(self.dest, members=selected_members)
                self.files = [safejoin(self.dest, member.filename)
