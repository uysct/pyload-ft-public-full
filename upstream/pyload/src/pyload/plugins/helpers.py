...
from ..core.utils.fs import safejoin
...
def copy_tree(src, dst, overwrite=False, preserve_metadata=False):
    ...
    src_root = os.path.realpath(src)
    dst_root = os.path.realpath(dst)

    if preserve_metadata and not exists(dst_root):
        return shutil.copytree(src_root, dst_root)

    for src_dir, dirs, files in os.walk(src_root, topdown=False):
        rel_dir = os.path.relpath(src_dir, src_root)
        dst_dir = dst_root if rel_dir == "." else safejoin(dst_root, rel_dir)
        ...
        for filename in files:
            src_file = safejoin(src_dir, filename)
            dst_file = safejoin(dst_dir, filename)
