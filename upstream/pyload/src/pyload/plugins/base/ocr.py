...
class BaseOCR(BasePlugin):
    ...
    def load_image(self, image):
        if Image is None:
            raise RuntimeError("OCR support requires Pillow (PIL), which is not available in this image")
    ...
    def run_tesser(
        ...
        try:
            tmp_tif = open(
                safejoin(self.pyload.tempdir, f"tmp_tif_{self.classname}.tif"),
                mode="wb",
            )
            tmp_tif.close()
            ...
            tmp_txt = open(
                safejoin(self.pyload.tempdir, f"tmp_txt_{self.classname}.txt"),
                mode="wb",
            )
            tmp_txt.close()
        ...
        if subset and (digits or lowercase or uppercase):
            ...
            with open(
                ...
                tmp_sub.write(b"tessedit_char_whitelist ")

                if digits:
                    tmp_sub.write(b"0123456789")
                if lowercase:
                    tmp_sub.write(b"abcdefghijklmnopqrstuvwxyz")
                if uppercase:
                    tmp_sub.write(b"ABCDEFGHIJKLMNOPQRSTUVWXYZ")
