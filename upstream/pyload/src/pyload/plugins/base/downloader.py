...
class BaseDownloader(BaseHoster):
    ...
    def _download(
        ...
        try:
            ...
            if self.req.code in (404, 410):
                if newname:
                    bad_file = fs.safejoin(os.path.dirname(filename), newname)
                else:
                    bad_file = filename
    ...
    def check_duplicates(self):
        ...
        dl_root_folder = self.pyload.config.get("general", "storage_folder")
        dl_package_folder = fs.safejoin(dl_root_folder, pack_folder)
        dl_file = fs.safejoin(dl_package_folder, self.pyfile.name)
        ...
        if self.pyload.config.get("download", "skip_existing"):
            ...
            while True:
                ...
                dl_file = fs.safejoin(dl_package_folder, name)
                if not exists(dl_file):
                    break
