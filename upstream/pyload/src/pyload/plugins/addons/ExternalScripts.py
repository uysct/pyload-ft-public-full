...
class ExternalScripts(BaseAddon):
    ...
    def make_folders(self):
        scripts_root = safejoin(self.pyload.userdir, "scripts")

        for folder in self.folders:
            folder_dir = safejoin(scripts_root, folder)
        ...
        for folder in self.folders:
            ...
            if os.path.isdir(folder_dir):
                for entry in os.listdir(folder_dir):
                    ...
                    file = safejoin(folder_dir, entry_name)
                    ...
                    if (
                        ...
                        or entry_name.endswith(".swp")
                    ):
                        continue

                    if not os.access(file, os.X_OK):
                        self.log_warning(
                            self._("Script `{}` is not executable").format(entry_name)
                        )
    ...
    def archive_processed(self, pypack):
        ...
        if self.pyload.config.get("general", "folder_per_package"):
            dl_folder = safejoin(dl_folder, pypack.folder)
    ...
    def package_deleted(self, pid):
        ...
        if self.pyload.config.get("general", "folder_per_package"):
            dl_folder = safejoin(dl_folder, pdata.folder)
