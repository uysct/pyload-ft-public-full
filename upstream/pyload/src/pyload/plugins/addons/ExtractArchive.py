...
from pyload.core.utils.fs import is_within_directory, safename, safejoin
...
class ExtractArchive(BaseAddon):
    ...
    def extract(
        ...
        for package_id in package_ids:
            ...
            pack_dl_folder = safejoin(
                dl_folder, pypack.folder
            )

            #: Determine output folder
            extract_folder = safejoin(
            ...
            files_ids = list(
                {
                    fdata["name"]: (
                        fdata["id"],
                        safejoin(pack_dl_folder, fdata["name"]),
                        extract_folder,
                    )
            ...
            if matched:
                if success:
                    ...
                    if self.config.get("delete") and self.pyload.config.get(
                        ...
                        if not is_within_directory(pack_dl_folder, extract_folder):
