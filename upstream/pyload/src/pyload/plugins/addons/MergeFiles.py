...
class MergeFiles(BaseAddon):
    ...
    def package_finished(self, pack):
        ...
        if self.pyload.config.get("general", "folder_per_package"):
            dl_folder = safejoin(dl_folder, pack.folder)

        for name, file_list in files.items():
            self.log_info(self._("Starting merging of"), name)

            final_path = safejoin(dl_folder, name)
            with open(final_path, mode="wb") as final_file:
                for splitted_file in file_list:
                    ...
                    try:
                        split_path = safejoin(dl_folder, splitted_file)
                        with open(split_path, "rb") as s_file:
                            size_written = 0
                            s_file_size = int(os.path.getsize(split_path))
                            while True:
